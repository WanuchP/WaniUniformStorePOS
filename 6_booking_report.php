...
if (isset($_GET['bill_id'])&&isset($_GET['status_booking'])) {
  ...
  header("location:6_booking_report.php");
  exit();
  }
?>   
<!DOCTYPE html>
              ...
              while ($row=$result->fetch_assoc()) {?>
                ...
                ?> 
                  <td>
                        <select onchange="status_update(this.options[this.selectedIndex].value,'<?php echo $row['bill_id'] ?>')" class="my-dropdown">  
                            <option value="Not Recieved Yet">Not Recieved Yet</option>  
                ...
                if($row["status_booking"]=="Recieved"){  ?>  
                  <td>
                        ...
                        <option value="Recieved">Recieved</option>                           
                        <option value="Not Recieved Yet">No Recieved Yet</option>
                        </select> 
                      </td>  
                  </tr>   
           <?php      }}  
...
<script type="text/javascript">  
      function status_update(value,bill_id){  
           let url = "http://win.ie.eng.chula.ac.th/~tuesday_2023/T1/Finished/6_booking_report.php";  
           window.location.href= url+"?bill_id="+bill_id+"&status_booking="+value;  
      }  
 </script> 

    <div class="bottom_line">
        <button type="button" class="button1" onclick="window.location.href='2_CashierDesktop.php'">
